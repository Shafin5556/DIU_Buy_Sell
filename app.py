...
def login():
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            "SELECT id, username, email, password, is_admin FROM users WHERE email = %s",
            (email,),
        )
        user = cursor.fetchone()
        cursor.close()
...
def index():
    ...
    cursor.execute(
        """SELECT id, image, description, price 
                       FROM posts 
                       WHERE user_id = %s""",
        (session["user_id"],),
    )
    ...
    for post in user_posts:
        ...
        cursor.execute(
            """SELECT users.username 
                           ...
                           WHERE post_interests.post_id = %s""",
            (post_id,),
        )
    ...
    return render_template(
        "index.html", user_posts=user_posts, post_interests=post_interests
    )
...
def buy():
    ...
    cursor.execute(
        """SELECT posts.id, posts.image, posts.description, posts.price, users.username, users.profile_pic
                       FROM posts 
    ...
    for post in posts:
        cursor.execute(
            """SELECT comments.comment_text, users.username, users.profile_pic 
                           FROM comments 
    ...
    cursor.execute(
        """SELECT post_id FROM post_interests WHERE user_id = %s""", (user_id,)
    )
    ...
    return render_template(
        "buy.html", posts=posts, comments=comments, user_has_interest=user_has_interest
    )
...
def sell():
    ...
    if request.method == "POST":
        ...
        cursor = connection.cursor()
        cursor.execute(
            "INSERT INTO posts (user_id, image, description, price) VALUES (%s, %s, %s, %s)",
...
def comment(post_id):
    ...
    cursor.execute(
        """SELECT comment_text, users.username, users.profile_pic 
                       FROM comments 
                       JOIN users ON comments.user_id = users.id 
                       WHERE comments.post_id = %s 
        ...
        return jsonify(
            {
                ...
                "profile_pic": (
                    new_comment[2] if new_comment[2] else "default-profile.png"
                ...
                "comments": [
                    {
                        ...
                        "username": comment[1],
                        "profile_pic": comment[2] or "default-profile.png",
                    }
...
def edit_comment(post_id):
    ...
    cursor.execute(
        """SELECT comments.id FROM comments 
                      JOIN users ON comments.user_id = users.id
                      WHERE comments.post_id = %s AND users.username = %s""",
        (post_id, username),
    )
...
def admin():
    ...
    cursor = connection.cursor()
    cursor.execute(
        """SELECT posts.id, posts.image, posts.description, posts.price, users.username
...
def profile():
    ...
    if request.method == "POST":
        ...
        password = request.form["password"]
        profile_pic = (
            request.files["profile_pic"] if "profile_pic" in request.files else None
        )
...
def add_interest(post_id):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT * FROM post_interests WHERE post_id = %s AND user_id = %s",
        (post_id, user_id),
    )
    ...
    cursor.execute(
        "INSERT INTO post_interests (post_id, user_id) VALUES (%s, %s)",
        (post_id, user_id),
    )
    connection.commit()
...
@app.route("/srs", methods=["GET", "POST"])
def srs():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            cursor = connection.cursor()
            cursor.execute(
                """INSERT INTO srs_files (file_name, file_type, file_path)
                   VALUES (%s, %s, %s)""",
            ...
            return redirect(url_for("srs"))

        else:
            flash("Invalid file type. Only PDF and DOC files are allowed.", "danger")

    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    cursor.execute("SELECT * FROM srs_files")
    files = cursor.fetchall()
    cursor.close()
    connection.close()

    return render_template("srs.html", files=files)


@app.route("/delete_srs/<int:file_id>", methods=["POST"])
def delete_srs(file_id):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    cursor.execute("SELECT file_path FROM srs_files WHERE id = %s", (file_id,))
    file = cursor.fetchone()

    if file:
        os.remove(file["file_path"])
        cursor.execute("DELETE FROM srs_files WHERE id = %s", (file_id,))
        connection.commit()
...
@app.route("/srs/files/download/<int:file_id>", methods=["GET"])
def download_srs(file_id):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    cursor.execute(
        "SELECT file_name, file_path FROM srs_files WHERE id = %s", (file_id,)
    )
    file = cursor.fetchone()
    cursor.close()
    connection.close()
...
@app.route("/srs/files/view/<int:file_id>", methods=["GET"])
def view_srs(file_id):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    cursor.execute(
        "SELECT file_name, file_path, file_type FROM srs_files WHERE id = %s",
        (file_id,),
    )
    file = cursor.fetchone()
    cursor.close()
    ...
    if file:
        ...
        return send_file(
            file_path, as_attachment=False, mimetype="application/octet-stream"
        )
    else:
        return "File not found", 404


@app.route("/srs/files", methods=["GET"])
def view_srs_files():
    ...
    cursor = connection.cursor(dictionary=True)
    cursor.execute(
        "SELECT id, file_name, file_type, file_path, uploaded_at FROM srs_files ORDER BY uploaded_at DESC"
    )
    files = cursor.fetchall()
    cursor.close()
    connection.close()

    return render_template("view_srs_files.html", files=files)
